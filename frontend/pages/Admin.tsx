...
import { Plus, Package, ShoppingBag, Trash2, Edit2, X, Save, Search, Filter, MessageCircle, Upload, Image as ImageIcon, MapPin } from 'lucide-react';
...
const ProductManager = () => {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert(error.message || 'Failed to upload image. Please try again or use image URL.');
    } finally {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {/* Upload File */}
                <div className="mb-3">
                  <label className="flex items-center gap-2 px-4 py-2 bg-brand-light text-brand-ocean rounded-lg cursor-pointer hover:bg-brand-cyan/10 transition-colors w-fit">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-cyan focus:border-brand-cyan outline-none"
                    placeholder="https://example.com/image.jpg or /uploads/images/product.jpg"
                  />
                  <p className="text-xs text-gray-400 mt-1">Supports both absolute URLs (http://...) and relative paths (/uploads/...)</p>
                </div>
              </div>
            </div>
                  ...
                  <tr key={product.id} className="hover:bg-gray-50 transition">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        <img 
                          src={getImageUrl(product.image) || 'https://via.placeholder.com/60'} 
                          alt={product.name}
...
const OrderManager = () => {
  ...
  useEffect(() => {
    ...
    // Poll every 5 seconds instead of 2s to reduce server load
            ...
            <div key={order.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition">
              ...
              {order.shippingAddress && (
                <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  ...
                  <p className="text-sm text-blue-900">{order.shippingAddress}</p>
                </div>
              )}
